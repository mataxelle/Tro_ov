<template>
  <div class="mt-5 py-5 px-5">
    ...
    <div v-if="!objects" class="text-center">
    ...
    <div v-if="error">ICI : {{ error.message }}</div>
      ...
      <div v-for="object in objects" :key="object.id" class="col">
        <div class="card h-100 shadow">
          <div class="card-body">
            ...
            <NuxtLink :to="`/objects/${object._id}`" role="button" class="btn btn-outline-success my-4"
              aria-label="Lire l'histoire de {{ object.name }}">Lire mon histoire
            </NuxtLink>
          </div>
...
import { useAsyncData, useLazyFetch } from '#imports';

const objectStore = useObjectStore();

const { data: objects, error, refresh } = useAsyncData('objects', async () => {
